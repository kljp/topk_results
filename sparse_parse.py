...
while True:
    ...
    if line.find('COMP=*.') != -1:
        ...
        if m is not None:
            ...
            cnt_comp = cnt_comp + 1
            comp = comp + float(m2)
    if line.find('COMM_TOTAL=*.') != -1:
        m = re.search('COMM_TOTAL=(.*?),', line)
        if m is not None:
            m2 = str(m.group(1))
            if float(m2) > 0.000 and float(m2) < 1.01:
                cnt_comm = cnt_comm + 1
                comm = comm + float(m2)
...
print('Prop = %f' % (prop / cnt_prop - comp / cnt_comp))
print('Comm = %f' % (comm / cnt_comm))
print('Comp = %f' % (comp / cnt_comp))
